...
scoreboard players remove @s[scores={player.sprinting=1}] player.stamina 5
execute unless score @s player.tired matches 1.. unless score @s player.sprinting matches 1 run scoreboard players add @s player.stamina 3
# cap it
...
scoreboard players add @s player.target_hunger 7
...
execute if score @s temp > @s player.target_hunger run effect give @s hunger 1 100

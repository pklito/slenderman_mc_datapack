execute as @a at @s run fill ~-14 ~-3 ~-14 ~14 ~14 ~14 air replace minecraft:light
...
scoreboard players set @a[gamemode=!spectator,nbt={SelectedItem:{components:{"minecraft:custom_data":{flashlight:true}}}}] player.holding_flashlight 1
scoreboard players set @a[gamemode=!spectator,nbt={SelectedItem:{components:{"minecraft:custom_data":{flashlight:true,dropped:true}}}}] item.picked_up_flashlight 1
...
execute as @a[gamemode=!spectator,scores={item.old_flashlight_id=1..,item.flashlight_state=2,item.threw_flashlight=0}] at @s unless score @s item.old_flashlight_id = @s item.flashlight_id run function lito:item/flashlight_off_ux_all
execute as @a[gamemode=!spectator,scores={item.old_flashlight_id=1..,item.flashlight_state=2,item.threw_flashlight=0}] at @s unless score @s item.old_flashlight_id = @s item.flashlight_id run scoreboard players set @s item.flashlight_state 1

execute as @a[gamemode=!spectator,scores={item.flashlight_state=1,player.holding_flashlight=1,item.picked_up_flashlight=0}] at @s run function lito:item/flashlight_on_ux

scoreboard players set @a[gamemode=!spectator,scores={item.flashlight_state=1,player.holding_flashlight=1}] item.flashlight_state 2
scoreboard players set @a[gamemode=!spectator,scores={item.flashlight_state=2,player.holding_flashlight=0}] item.flashlight_state 1
...
scoreboard players set @a[gamemode=!spectator,scores={item.flashlight_state=2,item.battery=1..}] player.flashing_light 1
...
execute if score #flashBlocks dev.config matches 1 as @a[scores={player.flashing_light=1, player.sprinting= 1}] at @s align xyz positioned ~0.5 ~ ~0.5 anchored eyes run function lito:light/running_angle
...
execute if score #flashCircle dev.config matches 1 run effect give @a[tag=user.playing,scores={player.flashing_light=1, player.sprinting=1}] minecraft:night_vision 2 0 true
